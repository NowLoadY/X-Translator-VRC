...
1. **启动自检机制**：程序每次启动时调用 `onboarding::resolve_startup_onboarding_state`。
   - 若检测到缺失任一核心前置资源（API Key、本地 ASR/翻译模型包、已启用的 TTS Audio8 模型、llama.cpp 运行时或 ONNX 加速库），自动将 `first_run` 置为 `true` 并停留在向导页（Step 1: Welcome）；
...
2. **向导 4 步骤流程**：
   ...
   - **Step 3: Optional TTS**：可选的 Audio8 语音克隆模型下载或跳过；
...
4. `runtime/native-runtime.json` 发布 llama 与 ONNX 的独立 backend、CUDA ABI、
   ONNX core、provider 目录、CUDA 目录、预加载顺序和 fallback reason。
5. 后端在任何 ONNX API 之前消费该清单。Audio8 Slow/Fast 必须同时成功使用 CUDA，
   否则整对回退 CPU；会话就绪事件报告实际 backend，而不是 UI 偏好。
